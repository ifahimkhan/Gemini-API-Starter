...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupRecyclerView() {
        ...
        responseAdapter = new ResponseAdapter(this, chatMessages);
        ...
        submitPromptButton.setOnClickListener(v -> {
            ...
            if (prompt.isEmpty()) {
                ...
                chatMessages.add(new ChatMessage(
                        getPrefixedText("", ""),
                        getPrefixedText("AI: ", getString(R.string.aistring))
                ));
                runOnUiThread(() -> {
                    responseAdapter.notifyItemInserted(chatMessages.size() - 1);
            ...
            generativeModel.generateContent(prompt, new Continuation<GenerateContentResponse>() {
                ...
                public void resumeWith(@NonNull Object o) {
                    ...
                    new Thread(() -> appDatabase.chatMessageDao().insert(new ChatMessageEntity(prompt, responseString))).start();

                    runOnUiThread(() -> {
                        progressBar.setVisibility(GONE);
                        SpannableStringBuilder userText = getPrefixedText("Me: ", prompt);
                        SpannableStringBuilder aiText = getPrefixedText("AI: ", responseString);
                        chatMessages.add(new ChatMessage(userText,aiText));
                        responseAdapter.notifyItemInserted(chatMessages.size() - 1);
                        recyclerView.scrollToPosition(chatMessages.size() - 1);
                        if (recyclerView.getItemAnimator() != null) {
                            recyclerView.getItemAnimator().setChangeDuration(250);
    ...
    private void loadChatHistory() {
        new Thread(() -> {
            List<ChatMessageEntity> savedMessages = appDatabase.chatMessageDao().getAllMessages();
            for (ChatMessageEntity entity : savedMessages) {
                ChatMessage chatMessage = new ChatMessage(
                        getPrefixedText("Me: ", entity.userMessage),
                        getPrefixedText("AI: ", entity.aiResponse));
